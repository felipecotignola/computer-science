...
void construtorQueue(){
	CircularQueue* queue=malloc(sizeof(CicularQueue));
	queue->array=malloc(5*sizeof(Veiculo));
	queue->inicio=0;
	queue->fim=0;
	queue->q=0;
	queue->c=5;
...
void enqueue(CircularQueue* queue,Veiculo* v){
	if(queue->q<queue->c){	
		queue->array[queue->fim]=v;
...
Veiculo* dequeue(CircularQueue* queue){
	if(queue->q>0){
		Veiculo* resp=queue->array[queue->inicio];
		queue->inicio=(queue->inicio+1)%queue->capacidade;
		return resp;
	}
}
void print(CircularQueue* queue){
	for(int i=queue->inicio;i!=queue->fim;i=(i+1)%array->c){
		printf("%s\n",formatVeiculo(queue->array[i];
	}
}
int main(){
	int tamanho;
	Veiculo* array=leitorCsv(&tamanho);	
	CircularQueue* queue=construtorQueue();
	char operacao;
	scanf("%c",&operacao);
