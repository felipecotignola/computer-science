...
Celula* construtorCelula(Veiculo* v){
...
void inserirInicio(List* l,Veiculo* v){
...
void inserirFim(List* l,Veiculo* v){
	Celula* c=construtorCelula();
	(*l).u->prox=c;
	l->u=c;
	l->q++;
}
void inserirPos(List* l,Veiculo* v,int pos){
	if(pos>=0 && pos<=l->q){
		Celula* c=construtorCelula();
		Celula* tmp=l->p;
		int i=0;
...
Veiculo* removerInicio(List* l){
	Celula resp=l->prox;
	Celula* tmp=resp->prox;
	l->porx=tmp;
	l->q--;
}
Veiculo* removerFim(List* l){
	Celula resp=l->u;
	Celula* tmp=(*l).p->prox;
	int i=0;
	while(i<l->q-1){
...
Veiculo* removerPos(List* l,int pos){
	if(pos>=0 && pos<l->q){
		Celula* resp;
		Celula tmp=(*l).p->prox;
		int i=0;
